...
class CasualAttention(nn.Module):
    def __init__(self, d_in, d_out, context_len, dropout, qkv_bias=False):
        ...
        # Upper triangular mask (causal: prevents attending to future tokens)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # split heads: (b, T, h, d_h) -> (b, h, T, d_h)
        ...
        # causal mask broadcast over batch & heads
        attn_scores = attn_scores.masked_fill(
            self.mask.bool()[:T, :T], float("-inf")
        )

        # scale by sqrt(d_h), softmax on last dim (keys dimension)
        attn_weights = torch.softmax(attn_scores / (self.head_dim ** 0.5), dim=-1)
        attn_weights = self.dropout(attn_weights)

        # context: (b, h, T, d_h) -> (b, T, h*d_h=d_out)
        context = (attn_weights @ V).transpose(1, 2).contiguous().view(b, T, self.d_out)
...
def main():
    ...
    print("Self attention v1:", sa_v1(inputs))

    torch.manual_seed(789)
    sa_v2 = SelfAttention_v2(d_in, d_out)
    print("Self attention in v2:", sa_v2(inputs))
    ...
    # Exercise 3.2
    mha2d = MultiHeadAttention(
            d_in, 4, context_length, 0.0, num_heads=2
            )
